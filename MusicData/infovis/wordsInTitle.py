...
text = str(list(df['title'])).replace(',', '').replace('[', '').replace("'", '').replace(']', '').replace('.', '').replace('Edition', '').replace('Vol ', '').replace('Volume', '')

# create world cloud
wordcloud = WordCloud(background_color = 'white', width = 840,  height = 620,colormap=cmap, max_words = 170).generate(text)

plt.figure(figsize=(8.4,6))
plt.imshow(wordcloud, interpolation = 'bilinear')
plt.axis('off')
plt.tight_layout(pad=0)
plt.title('Most Used Words in Title of Recommended Albums', fontweight="bold", fontfamily='serif', fontsize=16, color='black')
plt.text(25, 645, 'Author: Philipp Ackermann, philipp@metason.net;  Data Source: https://music.metason.net/greatRecords.html', fontsize=9, fontfamily='sans-serif', color=color3)
