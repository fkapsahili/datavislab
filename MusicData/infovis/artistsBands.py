...
ax.barh(ab_ratio.index,ab_ratio[0], 
        color=color1, alpha=0.9, label='Artist')
ax.barh(ab_ratio.index, ab_ratio[1], left=ab_ratio[0], 
        color=color2, alpha=0.9, label='Band')
...
for i in ab_ratio.index:
    ax.annotate(f"{round(ab_ratio[0][i]*100)}%", 
                   xy=(ab_ratio[0][i]/2, i),
                   va = 'center', ha='center',fontsize=40, fontweight='light', fontfamily='serif',
                   color='white')

    ax.annotate("Individual Artists", 
                   xy=(ab_ratio[0][i]/2, -0.25),
                   va = 'center', ha='center',fontsize=15, fontweight='light', fontfamily='serif',
                   color='white')

for i in ab_ratio.index:
    ax.annotate(f"{round(ab_ratio[1][i]*100)}%", 
                   xy=(ab_ratio[0][i]+ab_ratio[1][i]/2, i),
                   va = 'center', ha='center',fontsize=40, fontweight='light', fontfamily='serif',
                   color='black')
    ax.annotate("Music Groups", 
